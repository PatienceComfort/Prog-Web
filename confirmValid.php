...
<html lang="fr">
<?php
  	// validation des annotation et enregitrement dans les bases
	$str=$_SERVER['REQUEST_URI']; 
	$keywords = preg_split("/=/", $str);
	$id = $keywords[1]; 

	$update = pg_query($db,"UPDATE genegate.annotation SET statut='Validation' WHERE idSeq='".$id."'");
	$update2 = pg_query($db,"UPDATE genegate.annotation SET idValid2='".$_SESSION['mail']."' WHERE idSeq='".$id."'");
	$update3 = pg_query($db,"UPDATE genegate.transcrit SET annotee=TRUE WHERE idSeq='".$id."'");
...
<p> Confirmation de la validation ! <p>
<?php echo "<a href='mailto: ".$email."?subject=Confirmation Validation'> Avertir l'annotateur ? </a>"; // envoyer le mail ?>
</div>
<div class ="bottombar">
